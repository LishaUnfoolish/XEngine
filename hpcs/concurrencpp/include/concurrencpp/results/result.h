...
class result {
  ...
  static_assert(valid_result_type_v,
                "concurrencpp::result<type> - <<type>> should be "
                "now-throw-move constructable or void.");
 ...
 public:
  ...
  result(details::consumer_result_state_ptr<type> state) noexcept
      : m_state(std::move(state)) {}
  result(details::result_state<type>* state) noexcept : m_state(state) {}

  result& operator=(result&& rhs) noexcept {
    if (this != &rhs) { m_state = std::move(rhs.m_state); }
  ...
  result_status wait_for(
      std::chrono::duration<duration_type, ratio_type> duration) const {
  ...
  result_status wait_until(
      std::chrono::time_point<clock_type, duration_type> timeout_time) const {
...
class result_promise {
  ...
  static_assert(valid_result_type_v,
                "concurrencpp::result_promise<type> - <<type>> should be "
                "now-throw-move constructable or void.");
 ...
 private:
  ...
  void break_task_if_needed() noexcept {
    ...
    auto exception_ptr = std::make_exception_ptr(errors::broken_task(
        details::consts::k_broken_task_exception_error_msg));
 ...
 public:
  result_promise()
      : m_state(new details::result_state<type>()), m_result_retrieved(false) {}
  ...
  void set_result(argument_types&&... arguments) {
    ...
    static_assert(is_constructable,
                  "result_promise::set_result() - <<type>> is not "
                  "constructable from <<arguments...>>");
  ...
  void set_from_function(callable_type&& callable,
                         argument_types&&... args) noexcept {
    constexpr auto is_invokable =
        std::is_invocable_r_v<type, callable_type, argument_types...>;

    static_assert(
        is_invokable,
        "result_promise::set_from_function() - function(args...) is not "
        "invokable or its return type can't be used to construct <<type>>");
