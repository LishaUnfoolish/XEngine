...
class executor {
 private:
  template <class return_type, class executor_type, class callable_type,
            class... argument_types>
  static result<return_type> submit_bridge(executor_tag, executor_type&,
  ...
  static result<return_type> bulk_submit_bridge(
      details::executor_bulk_tag, std::vector<concurrencpp::task>& accumulator,
      callable_type callable) {
    co_return callable();
  }
 ...
 protected:
  ...
  static void do_post(executor_type& executor_ref, callable_type&& callable,
                      argument_types&&... arguments) {
    static_assert(std::is_invocable_v<callable_type, argument_types...>,
                  "concurrencpp::executor::post - <<callable_type>> is not "
                  "invokable with <<argument_types...>>");

    executor_ref.enqueue(details::bind_with_try_catch(
        std::forward<callable_type>(callable),
        std::forward<argument_types>(arguments)...));
  }

  template <class executor_type, class callable_type, class... argument_types>
  static auto do_submit(executor_type& executor_ref, callable_type&& callable,
                        argument_types&&... arguments) {
    static_assert(std::is_invocable_v<callable_type, argument_types...>,
                  "concurrencpp::executor::submit - <<callable_type>> is not "
                  "invokable with <<argument_types...>>");

    using return_type =
        typename std::invoke_result_t<callable_type, argument_types...>;
    return submit_bridge<return_type>(
        {}, executor_ref, std::forward<callable_type>(callable),
        std::forward<argument_types>(arguments)...);
  }

  template <class executor_type, class callable_type>
  static void do_bulk_post(executor_type& executor_ref,
  ...
  static std::vector<concurrencpp::result<return_type>> do_bulk_submit(
      executor_type& executor_ref, std::span<callable_type> callable_list) {
