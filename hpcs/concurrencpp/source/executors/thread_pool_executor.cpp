...
thread_pool_executor::thread_pool_executor(std::string_view pool_name, size_t pool_size, std::chrono::milliseconds max_idle_time) :
    derivable_executor<concurrencpp::thread_pool_executor>(pool_name), m_priority_cursor(0), m_idle_workers(pool_size),
    m_abort(false) {
...
void thread_pool_executor::enqueue(concurrencpp::task task) {
    ...
    const auto next_worker = m_priority_cursor.fetch_add(1, std::memory_order_relaxed) % m_workers.size();
