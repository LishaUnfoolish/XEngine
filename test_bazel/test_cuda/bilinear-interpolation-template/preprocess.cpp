...
// 根据比例进行缩放 (GPU版本)
template <typename T>
cv::Mat preprocess_gpu(cv::Mat &h_src, const int &tar_h, const int &tar_w,
                       Timer timer) {
  T *d_tar = nullptr;
  ...
  if (type == CV_8UC3) {
    timer.duration_gpu(
        "Resize(bilinear-letterbox-center, uint8) in gpu takes:");
  } else {
    timer.duration_gpu(
        "Resize(bilinear-letterbox-center, float32) in gpu takes:");
  }
...
template cv::Mat preprocess_gpu<u_int8_t>(cv::Mat &h_src, const int &tar_h,
                                          const int &tar_w, Timer timer);
template cv::Mat preprocess_gpu<float>(cv::Mat &h_src, const int &tar_h,
                                       const int &tar_w, Timer timer);
