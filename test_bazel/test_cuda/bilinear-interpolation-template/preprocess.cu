...
#include "stdio.h"
#include "test_bazel/test_cuda/bilinear-interpolation-template/utils.hpp"

template <typename T>
__global__ void resize_bilinear_BGR2RGB_shift_kernel(T* tar, uint8_t* src,
                                                     int tarW, int tarH,
                                                     int srcW, int srcH,
                                                     float scaled_w,
                                                     float scaled_h) {
...
template <typename T>
void resize_bilinear_gpu(T* d_tar, uint8_t* d_src, int tarW, int tarH, int srcW,
                         int srcH) {
...
template __global__ void resize_bilinear_BGR2RGB_shift_kernel<uint8_t>(
    uint8_t* tar, uint8_t* src, int tarW, int tarH, int srcW, int srcH,
    float scaled_w, float scaled_h);
template __global__ void resize_bilinear_BGR2RGB_shift_kernel<float>(
    float* tar, uint8_t* src, int tarW, int tarH, int srcW, int srcH,
    float scaled_w, float scaled_h);
template void resize_bilinear_gpu<uint8_t>(uint8_t* d_tar, uint8_t* d_src,
                                           int tarW, int tarH, int srcW,
                                           int srcH);
template void resize_bilinear_gpu<float>(float* d_tar, uint8_t* d_src, int tarW,
