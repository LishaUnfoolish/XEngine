...
*/
__global__ void MatmulSharedStaticConflictKernel(float *M_device,
                                                 float *N_device,
                                                 float *P_device, int width) {
  ...
  */
  ...
  int ty = threadIdx.y;
  int tx = threadIdx.x;
  /* 对于每一个P的元素，我们只需要循环遍历width / tile_width
   * 次就okay了，这里有点绕，画图理解一下*/
...
__global__ void MatmulSharedDynamicConflictKernel(float *M_device,
                                                  float *N_device,
                                                  float *P_device, int width,
                                                  int blockSize) {
  /*
  ...
  */
  ...
  for (int m = 0; m < width / blockSize; m++) {
    /* 这里为了实现bank conflict, 把tx与tx的顺序颠倒，同时索引也改变了*/
    deviceShared[tx * blockSize + ty] =
        M_device[x * width + (m * blockSize + ty)];
    deviceShared[stride + (tx * blockSize + ty)] =
        N_device[(m * blockSize + tx) * width + y];

    __syncthreads();

    for (int k = 0; k < blockSize; k++) {
      P_element += deviceShared[tx * blockSize + k] *
                   deviceShared[stride + (k * blockSize + ty)];
    }
    __syncthreads();
  }
...
*/
void MatmulSharedConflictOnDevice(float *M_host, float *N_host, float *P_host,
                                  int width, int blockSize, bool staticMem) {
  /* 设置矩阵大小 */
  ...
  /* 调用kernel来进行matmul计算,
   * 在这个例子中我们用的方案是：使用一个grid，一个grid里有width*width个线程 */
  dim3 dimBlock(blockSize, blockSize);
  dim3 dimGrid(width / blockSize, width / blockSize);
  if (staticMem) {
    MatmulSharedStaticConflictKernel<<<dimGrid, dimBlock>>>(M_device, N_device,
                                                            P_device, width);
  } else {
    MatmulSharedDynamicConflictKernel<<<dimGrid, dimBlock, sMemSize, nullptr>>>(
        M_device, N_device, P_device, width, blockSize);
  }
